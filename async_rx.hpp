...
{
  // Constructor
  Async_rx_channel( const std::string& instance_name )
  : sc_core::sc_prim_channel( instance_name.c_str() )
  {
  }

  // Destructor
  ~Async_rx_channel( void )
  {
  }
  ...
  void wait_unless_available( void ) override
  {
    if ( is_empty() ) sc_core::wait( m_event );
  }
  ...
  const sc_core::sc_event& default_event( void ) const override
  {
    return m_event;
  }
  ...
  bool nb_peek( Async_payload<T>*& payload_ptr ) const override
  {
    std::lock_guard<std::mutex> the_lock( m_queue_mutex );
    if ( m_queue.empty() ) return false;
  ...
  void peek( Async_payload<T>*& payload_ptr ) const override
  {
    wait_unless_available();
